/* Slowlog慢日志 implements a system that is able to remember the latest N
    ...
    for (j = 0; j < slargc; j++) {
        ...
        if (slargc != argc && j == slargc - 1) {
            se->argv[j] = createObject(REDIS_STRING,
                                       sdscatprintf(sdsempty(), "... (%d more arguments)", argc - slargc + 1));
        } else {
...
void slowlogCommand(redisClient *c) {
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "reset")) {
        ...
        // 遍历日志,取出指定数量的日志, 从头开始遍历, 最新的在最前面, id更大
