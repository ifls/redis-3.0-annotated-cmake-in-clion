 ...
 * support partial sorting of ranges for Redis. 快速排序 libc的实现 拿出来 改了
 */
...
static inline char *med3(char *a, char *b, char *c, int (*cmp)(const void *, const void *)) {
    ...
    return cmp(a, b) < 0 ? (cmp(b, c) < 0 ? b : (cmp(a, c) < 0 ? c : a)) : (cmp(b, c) > 0 ? b : (cmp(a, c) < 0 ? a
                                                                                                               : c));
}
...
// 参考 https://blog.csdn.net/guodongxiaren/article/details/45567291
