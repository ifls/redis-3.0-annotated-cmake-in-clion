...
/* 默认的服务器配置值 */
#define REDIS_DEFAULT_HZ        10      /* 时钟终端次数 Time interrupt calls/sec. */
#define REDIS_MIN_HZ            1
#define REDIS_MAX_HZ            500
#define REDIS_SERVERPORT        6379    /* TCP port */
#define REDIS_TCP_BACKLOG       511     /* TCP listen backlog tcp连接队列 */
...
typedef struct redisDb {
    ...
    // 键的过期时间字典,字典的键为键,字典的值为过期事件 UNIX 时间戳
...
typedef struct redisClient {
    ...
    // 记录被客户端执行的命令
    struct redisCommand *cmd, *lastcmd;  // 当前命令, 上一条命令
...
struct sharedObjectsStruct {  // czero == "0\r\n" cone = "1\r\n"
...
struct redisServer {
    ...
    dict *commands;             /* 存放命令 hash表 Command table */
    ...
    int tcp_backlog;            /* TCP listen() backlog tcp 连接队列 */
    ...
    int dbnum;                      /* 总的db数量 Total number of configured DBs */
    ...
    list *unblocked_clients; /* 未阻塞的客户端链表 list of clients to unblock before next loop */
...
extern struct redisServer server;  // 全局服务器变量
extern struct sharedObjectsStruct shared;  // 可复用公用对象
