...
void setTypeConvert(robj *setobj, int enc) {
    ...
    // 确认类型和编码正确
    redisAssertWithInfo(NULL, setobj, setobj->type == REDIS_SET && setobj->encoding == REDIS_ENCODING_INTSET);
...
void smoveCommand(redisClient *c) {
    ...
    // 或者目标集合存在、但是类型错误
    // 那么直接返回
    if (checkType(c, srcset, REDIS_SET) || (dstset && checkType(c, dstset, REDIS_SET)))
        return;
...
void sunionDiffGenericCommand(redisClient *c, robj **setkeys, int setnum, robj *dstkey, int op) {
    ...
    if (!dstkey) {
        ...
        if (setTypeSize(dstset) > 0) {
            ...
            // 返回结果集的基数
            addReplyLongLong(c, setTypeSize(dstset));
            notifyKeyspaceEvent(REDIS_NOTIFY_SET, op == REDIS_OP_UNION ? "sunionstore" : "sdiffstore", dstkey,
                                c->db->id);
...
void sscanCommand(redisClient *c) {
    ...
    if ((set = lookupKeyReadOrReply(c, c->argv[1], shared.emptyscan)) == NULL || checkType(c, set, REDIS_SET))
        return;
