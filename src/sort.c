/* SORT command and helper functions. 实现 sort命令
...
void sortCommand(redisClient *c) {
    ...
    if (sortval && sortval->type != REDIS_SET && sortval->type != REDIS_LIST && sortval->type != REDIS_ZSET) {
     ...
     * 因为有序集合的成员已经是有序的了.
	 */
    if ((storekey || c->flags & REDIS_LUA_CLIENT) && (dontsort && sortval->type != REDIS_ZSET)) {
        /* Force ALPHA sorting */
        // 强制 ALPHA 排序
        dontsort = 0;
        alpha = 1;
	 ...
	 */
    if (sortval->type == REDIS_ZSET && dontsort && (start != 0 || end != vectorlen - 1)) {
